...
def sbrun(*args, env_override=None, cwd=None, stdin=None, allow_fail=False):
    env = {**os.environ, "SBRUN_ALLOW_STDIO_REDIRECTS": "1",
           "XDG_CONFIG_DIRS": "/nonexistent", "XDG_CONFIG_HOME": "/nonexistent"}
    if env_override: env.update(env_override)
    r = subprocess.run([str(SBRUN), *args], capture_output=True, text=True,
...
def test_help():
    r = sbrun("--help")
    for flag in ("--write PATH", "--env-dir VAR", "--unset-env VAR", "--config PATH", "--no-config"):
...
def test_sbrun_active():
    assert sbrun("--no-config", "--", "sh", "-c", "echo $SBRUN_ACTIVE").stdout.strip() == "1"

def test_tmpdir():
    assert sbrun("--no-config", "--", "sh", "-c", "echo $TMPDIR").stdout.strip() == "/tmp"
...
def test_unset_env():
    r = sbrun("--no-config", "--unset-env", "SECRET", "--", "sh", "-c", "echo ${SECRET:-gone}",
              env_override={"SECRET": "hunter2"})
    assert r.stdout.strip() == "gone"

def test_direct_command():
    assert sbrun("--no-config", "--", "sh", "-c", "echo direct").stdout.strip() == "direct"
...
def test_tmp_writable():
    r = sbrun("--no-config", "-w", "/tmp", "--", "sh", "-c",
              "echo ok > /tmp/sbrun-pytest && cat /tmp/sbrun-pytest")
    assert r.stdout.strip() == "ok"

def test_extra_dir_writable(tmp):
    extra = tmp / "extra"
    extra.mkdir()
    r = sbrun("--no-config", "-w", str(extra), "--", "sh", "-c",
              f"touch {extra}/ok && echo ok", cwd=tmp)
    assert r.stdout.strip() == "ok"

def test_subdir_creation(tmp):
    r = sbrun("--no-config", "--", "sh", "-c",
              "mkdir -p a/b/c && echo hello > a/b/c/f && cat a/b/c/f", cwd=tmp)
    assert r.stdout.strip() == "hello"

def test_deny_write_shows_error():
    home = os.environ.get("HOME", "/tmp")
    r = sbrun("--no-config", "--", "sh", "-c",
              f"touch {home}/.sbrun-nope 2>&1", allow_fail=True)
...
def test_read_system_files():
    r = sbrun("--no-config", "--", "sh", "-c", "cat /etc/os-release 2>/dev/null | head -1 || cat /etc/hostname")
...
def test_explicit_config(tmp):
    ...
    r = sbrun("--config", str(cfg), "--", "sh", "-c",
              "echo ok > /tmp/sbrun-cfg && cat /tmp/sbrun-cfg", cwd=tmp)
    assert r.stdout.strip() == "ok"
...
def test_xdg_config(tmp):
    ...
    r = sbrun("--", "sh", "-c", "echo ok > /tmp/sbrun-xdg && cat /tmp/sbrun-xdg",
              cwd=tmp, env_override={"XDG_CONFIG_HOME": str(tmp / "xdg"), "XDG_CONFIG_DIRS": "/nonexistent"})
    assert r.stdout.strip() == "ok"
...
def test_auto_creates_default_config(tmp):
    ...
    sbrun("--", "sh", "-c", "echo ok", cwd=tmp,
          env_override={"XDG_CONFIG_HOME": str(xdg), "XDG_CONFIG_DIRS": "/nonexistent"})
    assert cfg_path.exists()
    content = cfg_path.read_text()
    assert "version = 1" in content
    assert "optional_write" in content
    # Second run should not print creation message
    r = sbrun("--", "sh", "-c", "echo ok", cwd=tmp,
              env_override={"XDG_CONFIG_HOME": str(xdg), "XDG_CONFIG_DIRS": "/nonexistent"})
...
def test_redirect_to_home_blocked(tmp):
    ...
    with open(target, "w") as f:
        r = subprocess.run([str(SBRUN), "--no-config", "--", "sh", "-c", "echo blocked"],
