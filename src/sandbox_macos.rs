...
unsafe extern "C" {
    ...
    fn sandbox_compile_string(profile: *const c_char, params: *mut c_void, errorbuf: *mut *mut c_char) -> *mut c_void;
    fn sandbox_free_profile(profile: *mut c_void);
    fn sandbox_apply(profile: *mut c_void) -> c_int;
}

pub fn apply(profile_text: &str, workdir: &Path, tty: &Path, histfile: Option<&Path>) -> Result<()> {
    ...
    let profile_text = CString::new(profile_text).map_err(|_| Error::Usage("sandbox profile contains NUL bytes".into()))?;
...
impl Params {
    fn new() -> Result<Self> {
        ...
        if raw.is_null() {
            return Err(Error::io("sandbox_create_params", std::io::Error::last_os_error()));
        }
        Ok(Self { raw, storage: Vec::new() })
    }

    fn set(&mut self, key: &str, value: CString) -> Result<()> {
        let key = CString::new(key).map_err(|_| Error::Usage("sandbox key contains NUL bytes".into()))?;
        ...
        if rc != 0 {
            return Err(Error::io("sandbox_set_param", std::io::Error::last_os_error()));
        }
        Ok(())
    }
    ...
    fn compile(&self, profile: &CStr) -> Result<Profile> {
        ...
        if compiled.is_null() {
            let detail = if errorbuf.is_null() {
                ...
                unsafe { CStr::from_ptr(errorbuf) }.to_string_lossy().into_owned()
            };
